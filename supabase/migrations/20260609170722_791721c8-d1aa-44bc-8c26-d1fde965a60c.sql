...
BEGIN
  -- Skip logging entirely if the parent client no longer exists
  -- (happens during cascade delete from clients)
  SELECT EXISTS(SELECT 1 FROM public.clients WHERE id = COALESCE(NEW.client_id, OLD.client_id))
...
DECLARE
  ...
  v_client_exists boolean;
BEGIN
  -- Skip logging if parent client no longer exists (cascade delete)
