...
serve(async (req) => {
  ...
  try {
    const { type, ctype, idea, model, referenceImages, format } = await req.json();
    ...
    if (type === "content") {
      const promptFn = PROMPTS[ctype || "linkedin"];
      if (!promptFn) throw new Error(`Unknown content type: ${ctype}`);
      const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
        method: "POST",
        headers: { Authorization: `Bearer ${LOVABLE_API_KEY}`, "Content-Type": "application/json" },
        body: JSON.stringify({ model: aiModel, messages: [{ role: "system", content: "Du bist ein professioneller Social Media Content Creator." }, { role: "user", content: buildUserContent(promptFn(idea) + imageContext, referenceImages) }] }),
      });
