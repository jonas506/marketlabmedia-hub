...
import { Flag, ArrowUpDown, ChevronUp, ChevronDown } from "lucide-react";
...
const TaskListView: React.FC<Props> = ({ tasks, clientMap, teamMap, todayStr, onSelect }) => {
  ...
  const SortHeader: React.FC<{ k: SortKey; children: React.ReactNode; className?: string }> = ({ k, children, className }) => (
    <button
      ...
      className={cn(
        "flex items-center gap-1 text-[10px] font-mono uppercase tracking-wide text-muted-foreground hover:text-foreground transition-colors",
        className
      )}
    >
      {children}
  ...
  return (
    <div className="rounded-xl border border-border/50 bg-surface-elevated/40 overflow-hidden">
      {/* Header */}
      <div className="grid grid-cols-[32px_1fr_180px_160px_120px_110px_110px] gap-3 px-3 py-2.5 border-b border-border/50 bg-background/30">
        <div />
        ...
        {sorted.map(t => {
          ...
          return (
            <div
              key={t.id}
              className={cn(
                "grid grid-cols-[32px_1fr_180px_160px_120px_110px_110px] gap-3 px-3 py-2 items-center hover:bg-muted/20 transition-colors cursor-pointer",
            ...
            >
              ...
              <div className={cn("text-sm font-medium truncate", t.is_completed && "line-through")}>
                {t.title}
              </div>
